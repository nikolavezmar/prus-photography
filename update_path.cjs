...
function rdp(points, epsilon) {
    ...
    for (let i = 1; i < end; i++) {
        const dist = ptLineDist(points[i], points[0], points[end]);
...
// 87 points is exactly halfway between 63 and 121 points.
const simplified = rdp(pts, 2.0);
console.log(`Simplified from ${pts.length} to ${simplified.length} points.`);

const n = simplified.length - 1;
const tension = 0.20; // Slightly tighter curve to avoid ballooning
...
const regex = /<g id=\"lingerSilhouette\">\s*<path\s+d=\"(.*?)\"\s+fill=\"#111\"\s*\/>\s*<\/g>/is;
if (!regex.test(html)) {
    console.error('REGEX FAILED to find the silhouette path in index.html');
    process.exit(1);
}

html = html.replace(regex, `<g id="lingerSilhouette">\n          <path d="${path}" fill="#111" />\n        </g>`);
fs.writeFileSync('index.html', html);
console.log('Path replaced successfully in index.html with epsilon 2.0!');
